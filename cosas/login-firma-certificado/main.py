...
def register(username: str, password: str):
    ...
    write(encrypt(seed, password.encode("utf-8")), "dykeiichi.key")
    print(signed)
    write(signed, "dykeiichi.cer")

def login(password: str):
    seed: bytes = decrypt(read("dykeiichi.key"), password.encode("utf-8"))
    signed_raw: bytes = read("dykeiichi.cer")
    print(signed_raw)
